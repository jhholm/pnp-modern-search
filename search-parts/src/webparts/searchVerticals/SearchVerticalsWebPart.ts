    ...
    public render(): void {
        ...
        if (this.properties.verticals.length > 0) {
            ...
            if (this.properties.defaultVerticalQuerystringParam) {
                ...
                const defaultQueryVal: string = queryParms.getValue(this.properties.defaultVerticalQuerystringParam);
                if (defaultQueryVal) {
                    const defaultSelected: ISearchVertical = find(this.properties.verticals, v => v.tabName.toLowerCase() == decodeURIComponent(defaultQueryVal).toLocaleLowerCase());
                    if (defaultSelected) {
